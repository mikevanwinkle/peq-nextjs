...
function getApiUrl(resource, params={}) {
  console.log(`${API_HOST}/api/${resource}?${querystring.stringify(params)}`)
...
async function getEntities(params={}) {
	return axios.get(`${API_HOST}/api/entities/all/?${querystring.stringify(params)}`, {headers: {
			"Cache-Control": "max-age=1000",
			"Content-Type": "application/json"
	}})
}
...
async function getClusters() {
  return axios.get('https://politicseq-live.s3.amazonaws.com/all-clusters.json')
}
